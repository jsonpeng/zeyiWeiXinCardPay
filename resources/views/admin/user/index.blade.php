            ...
            <div class="box-body">
                <form id="projects_search">
                    ...
                    <div class="form-group col-lg-3 col-md-4 col-sm-6 col-xs-6">
                        <label for="snumber">会员等级</label>
                            <select name="user_level" class="form-control">
                                    <option value="0" @if(!array_key_exists('user_level',$input)) selected="selected" @endif>全部</option>
                                @foreach ($userLevels as $item)
                                    <option value="{!! $item->id !!}" @if(array_key_exists('user_level', $input)) @if($item->id==$input['user_level']) selected="selected" @endif @endif>{!! $item->name !!}</option>
                                @endforeach
                            </select>
                    </div>
