...
class ProjectRepository extends BaseRepository
{
    ...
    //获取初始的项目列表
    public function getProjects($project='项目',$skip = 0,$take = 18,$page_times=1){
        return Cache::remember('zcjy_project_default_list'.$page_times.'_'.$project.$skip.'_'.$take, Config::get('web.shrottimecache'), function() use ($project,$skip,$take) {
           try {
            return Project::where('type',$project)->where('auth_status','通过')->where('status','正常')->orderBy('created_at','desc')->skip($skip)->paginate($take);
            } catch (Exception $e) {
                return [];
            }
         });
